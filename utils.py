...
def get_datafile(filename=None, encoding='utf-8'):
    if(os.path.isdir(filename)):
        filenames = [name for name in os.listdir(
            filename) if os.path.isfile(os.path.join(filename, name))]
        # self.data = []
        # training_data_dir = self.filename
        for name in filenames:
            try:
                for line in open(os.path.join(filename, name), 'rt', encoding=encoding):
                    for word in line.split():
                        yield word
                    yield '<eos>'
        ...
        for line in open(filename, 'rt', encoding=encoding):
            for word in line.split():
                yield word
            yield '<eos>'

# def get_develfile(filename=None, encoding='utf-8'):
...
def findMostPossible(a, options, temperature=1.0):
    # find most possible word
    a = np.log(a) / temperature
    a = np.exp(a) / np.sum(np.exp(a))
    return options[np.argmax(a[options])]
